    ...
    (async () => {
      try {
        ...
        const fetchXmlCandidate =
          (typeof launchContext.fetchXml === "string" && launchContext.fetchXml) ||
          (typeof launchContext.fetchxml === "string" && launchContext.fetchxml) ||
        ...
        if (!fetchXml) {
          addLog("Invocation context was provided but no fetchXml payload was found", "warning");
          return;
        }

        addLog("Invocation payload received. Running FetchXML query...", "info");
        viewModel.selectedView = undefined;
        viewModel.fetchXml = fetchXml;
        viewModel.fetchFields = [];

        const tableMatch = fetchXml.match(/<entity\s+name=['\"]([^'\"]+)['\"]/i);
        ...
        if (tableLogicalName) {
          ...
          const matchedTable = viewModel.tables.find((t) => t.logicalName === tableLogicalName);
          if (matchedTable) {
            viewModel.selectedTable = matchedTable;
            if (!matchedTable.fields || matchedTable.fields.length === 0) {
              matchedTable.fields = await dvSvc.getFields(matchedTable.logicalName);
            }
          } else {
            addLog(`Table ${tableLogicalName} from FetchXML was not found in this environment`, "warning");
          }
        }
        ...
        try {
          ...
          await window.toolboxAPI.invocation.returnData({
            status: "success",
            recordCount: Array.isArray(viewModel.data) ? viewModel.data.length : 0,
          });
